...
def fill_holes(image):
    """fill_holes for numpy array with multiple values"""
    image_processed = np.zeros(image.shape, dtype=image.dtype)
    objects = []
    values = list(np.unique(image)[1:])
    for i in values:
        objects.append(image == i)
    with multiprocessing.Pool(None) as p:
        objects = p.map(binary_imfill, objects)
    for i, obj in enumerate(objects):
        image_processed = np.add(image_processed, obj*i)
...
class CellSegmentator(object):
    ...
    def label_cells(self, images, generator=False):
        ...
        def _postprocess(n_labels, c_prediction):
            ...
            if self.scale_factor != 1.0:
                c_prediction = skimage.transform.rescale(c_prediction, 1/self.scale_factor)
            ...
            cell_label = skimage.morphology.watershed(-distance, n_labels, mask=skele)
...
if __name__ == '__main__':
    seg = CellSegmentator('./weights/dpn_unet_f0/dpn_unet_best.pth', './weights/nuclei_dpn_unet_f0/dpn_unet_best.pth')
    seg.label_cells(['./test_image'])