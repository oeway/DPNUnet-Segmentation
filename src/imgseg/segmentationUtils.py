...
# IMPORTS to create polygons from label images
...
def labels_to_polygon(img_label,label=None,simplify_tol=0, plot_simplify=False, save_name=None):
    '''
   ...
   3. Polygons will be saved in geojson format, which can be read by ImJoys'
    ...
    Args:
        img_label (2D numpy array): image wiht segmentation masks. Background is 0,
    ...
    for obj_int in np.nditer(ind_objs):
        ...
        contour = measure.find_contours(img_label_loop, 0.5)

        # Proceeed only if one contour was found
        if len(contour) == 1:

            contour_asNumpy = contour[0][:, np.argsort([1, 0])]
            contour_asNumpy[:, 1] = np.array([img_label.shape[0]-1 - h[0] for h in contour[0]])   # -1 is necessary since indexing starts at 0
