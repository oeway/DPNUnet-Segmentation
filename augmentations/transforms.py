...
class HueSaturationValue(ImageOnlyTransform):
    ...
    def apply(self, image, hue_shift=0, sat_shift=0, val_shift=0):
        assert image.dtype == np.uint8 or self.hue_shift_limit[0] < 1.
...
class Binarize(ImageOnlyTransform):
    def __init__(self, channels=None,prob=0.5):
        super().__init__(prob)
        self.channels = channels

    def apply(self, image, **params):
        return F.binarize(image, channels=self.channels)


class Quantize(ImageOnlyTransform):
    def __init__(self, im, nK=5):
        self.nK = nK
        
    def apply(self, im, **params):
        return F.quantize(im, nK=self.nK)
...
class ToTensor(BasicTransform):
    def __init__(self, target_channel_num=1, activation='sigmoid', normalize=None):
        super().__init__(1.)
    ...
    def __call__(self, **kwargs):
        ...
        if 'mask' in kwargs.keys():
            if self.activation == 'linear':
                kwargs.update({'mask': F.target_to_tensor(kwargs['mask'])})
            elif self.activation == 'sigmoid':
                kwargs.update({'mask': F.mask_to_tensor(kwargs['mask'], self.target_channel_num, True)})
            elif self.activation == 'softmax':
                kwargs.update({'mask': F.mask_to_tensor(kwargs['mask'], self.target_channel_num, False)})
            else:
                raise NotImplementedError
        return kwargs
...
def aug_victor_original(prob=0.9):
    return Compose([
        OneOf([
        ...
        ShiftScaleRotate(shift_limit=.0625, scale_limit=0.4, rotate_limit=90, prob=.7),
        OneOf([
            Distort1(prob=.2),
            Distort2(prob=.2),
...
def aug_victor(prob=0.9):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=.0625, scale_limit=0.4, rotate_limit=180, prob=.7),
        OneOf([
            Distort1(prob=.2),
            Distort2(prob=.2),
            ElasticTransform(prob=.2),
...
def aug_binarize(prob=1.):
    return Compose([
        ...
        Binarize(prob=prob)
        #Binarize(channels=[2], prob=prob)
        ], prob=prob)

def val_binarize(prob=1.):
    return Compose([               
        Binarize(prob=prob)
        #Binarize(channels=[2], prob=prob)
        ], prob=prob)
