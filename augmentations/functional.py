...
def mask_to_tensor(mask, target_channel_num, sigmoid):
...
def target_to_tensor(im):
    if im.dtype == np.uint16:
        im = im / np.iinfo(im.dtype).max
    else:
        im = im / (255. if im.dtype == np.uint8 else 1)
    if im.ndim == 2:
        im = im[:,:,np.newaxis]
    tensor = torch.from_numpy(np.moveaxis(im, -1, 0).astype(np.float32))
    return tensor

def binarize(im, channels=None):
    blur = cv2.GaussianBlur(im,(5,5),0)
    binarized_im = []
    if channels is None:
        channels = list(range(im.shape[2]))
    for channel in channels:
        ret, ch = cv2.threshold(blur[:,:,channel],0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        blur[:,:,channel] = ch
    binarized_im = blur 
    return binarized_im

def quantize_kmeans(im, K = 5):
    ...
    # Convert back into uint8, and reshape to original image
