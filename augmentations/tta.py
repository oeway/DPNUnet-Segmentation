...
class TTAOp:
    ...
    def __call__(self, model, batch):
        if batch.device == 'cpu':
            forwarded = torch.autograd.Variable(torch.from_numpy(self.forward(batch.numpy()))).to(batch.device)
        else:
            forwarded = torch.autograd.Variable(torch.from_numpy(self.forward(batch.to('cpu').numpy()))).to(batch.device)
...
transforms = [HFlip, VFlip, Transpose, HVFlip, TransposeHFlip, TransposeVFlip, TransposeHVFlip]
