...
class AbstractModel(nn.Module):
    ...
    def initialize_encoder(self, model, model_url, input_channel_num_changed=False):
...
class EncoderDecoder(AbstractModel):
    def __init__(self, target_channel_num, input_channel_num=3, encoder_name='resnet34'):
        ...
        self.final = self.make_final_classifier(self.filters[0], target_channel_num)

        self._initialize_weights()

        encoder = encoder_params[encoder_name]['init_op'](in_channels=input_channel_num)
        ...
        if encoder_params[encoder_name]['url'] is not None:
            self.initialize_encoder(encoder, encoder_params[encoder_name]['url'], input_channel_num != 3)
    ...
    def make_final_classifier(self, in_filters, target_channel_num):
        return nn.Sequential(
            ...
            # nn.ReLU(inplace=True),
            nn.Conv2d(in_filters, target_channel_num, 3, padding=1)
        )
